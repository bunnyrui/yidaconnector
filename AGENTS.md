# YidaConnector — AI Agent 开发指引

YidaConnector 是宜搭低代码平台的 CLI 桥接层：`bin/yida.js`（CommonJS）把命令分发到 `lib/` 下各模块。本文件只记录容易踩坑的约定，不重复文件名即可推断的内容。更完整的介绍见 `README.md` / `CLAUDE.md`。
...
- **`lib/mcp/` ≠ `mcp-app/`**：`lib/mcp/server.js` 是 CLI 自带的 MCP JSON-RPC 服务（`yidaconnector mcp` 调用）；`mcp-app/` 是 TypeScript 写的可交互 MCP App（Claude/ChatGPT UI host 用）。
...
1. `lib/core/command-manifest.js` —— 命令元数据（用法、描述 key、output 类型、aliases）。这是 `yidaconnector --help` 与 `yidaconnector commands`（输出 JSON，供 AI agent 消费的命令清单）的**唯一数据源**。
...
**Codex 登录 flag 语义易混：**
- `yidaconnector login`：默认缓存优先；无缓存走本地 Chrome/Edge/Chromium CDP；CDP 不可用则返回 AI 对话框二维码 handoff。默认**不**依赖 Playwright。
- `yidaconnector login --browser`：CDP 不可用时才用 Playwright 兜底。
- `yidaconnector login --qr`：强制终端二维码（测试二维码链路专用）。
...
- `yidaconnector copy` 在**空目录**中会直接把 `project/` 内容铺入当前目录（不创建 `project/` 子层级）。
- 悟空通过手动上传技能包安装；`postinstall` **不**会自动装 `yida-skills/`。

## yida-skills 双输出架构

- **源码态**在 `yida-skills/`（与历史安装路径、Codex/YidaConnector 插件兼容）；**对外发布的悟空 zip** 由 `npm run build:skills` 生成到 `dist/skills/yidaconnector/`（同时输出 `yidaconnector-skills.zip`）。
...
- 环境/登录问题：先 `yidaconnector env` 确认检测，再看 `lib/auth/`。
