...
function createCdnClient(config) {
  try {
    ...
    warn(t('cdn.cdn_sdk_required'));
    warn(t('cdn.run_npm_install', '@alicloud/cdn20180510 @alicloud/openapi-client'));
    process.exit(1);
  }
}
...
async function performRefresh(options) {
  ...
  if (!valid) {
    warn(t('cdn.config_incomplete'));
    warn(t('cdn.missing_fields', missing.join(', ')));
    warn(t('cdn.run_config_init'));
    process.exit(1);
  }
  ...
  if (options.file) {
    if (!fs.existsSync(options.file)) {
      warn(t('cdn.file_not_found', options.file));
      process.exit(1);
    }
  ...
  try {
    ...
    warn(t('cdn.quota_query_failed', error.message));
  }
  ...
  if (urls.length > 0) {
    ...
    try {
      ...
      warn(t('cdn.refresh_urls_failed', error.message));
  ...
  if (options.paths.length > 0) {
    ...
    try {
      ...
      warn(t('cdn.refresh_paths_failed', error.message));
...
async function run(args) {
  ...
  try {
    ...
    warn(t('cdn.refresh_error', error.message));
    process.exit(1);
  }
}
