...
function createOssClient(config) {
  try {
    ...
    warn(t('cdn.oss_sdk_required'));
    warn(t('cdn.run_npm_install', 'ali-oss'));
    process.exit(1);
  }
}
...
async function performUpload(options) {
  ...
  for (const filePath of imageFiles) {
    ...
    try {
      ...
      warn(t('cdn.upload_failed', fileName, error.message));
    }
  }
...
async function run(args) {
  ...
  try {
    ...
    warn(t('cdn.upload_error', error.message));
    process.exit(1);
  }
}
