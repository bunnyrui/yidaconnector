...
'use strict';
const { fail, warn, error } = require('../core/chalk');
...
function parseArgs(args) {
  ...
  if (!isUpdateMode) {
    if (args.length < 2) {
      fail('错误: 创建连接器需要提供名称和域名');
      showUsage();
      process.exit(1);
    }
    if (args[0].startsWith('--')) {
      fail(`错误: 第一个参数 "${args[0]}" 看起来是一个选项，应该是连接器名称`);
      process.exit(1);
    }
    if (args[1].startsWith('--')) {
      fail(`错误: 第二个参数 "${args[1]}" 看起来是一个选项，应该是域名`);
      process.exit(1);
    }
  }
...
async function run(args) {
  ...
  if (!options.operationsFile) {
    fail('错误: 创建连接器必须提供 --operations 参数');
    fail('宜搭平台不允许创建没有执行动作的连接器');
    process.exit(1);
  }
