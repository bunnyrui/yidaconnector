...
async function run(args) {
  ...
  if (!options.connectorId || !options.actionId) {
    fail('请提供 --connector-id 和 --action 参数');
    showUsage();
    process.exit(1);
  }
  ...
  if (!connector) {
    fail(`未找到连接器 ID: ${options.connectorId}`);
    process.exit(1);
  }
  ...
  console.log('📤 请求参数');
  console.log('──────────────────────────────────────────────────');
  console.log('Headers:', safeJsonStringify(testParams.header));
  console.log('Query:', safeJsonStringify(testParams.query));
  console.log('Body:', safeJsonStringify(testParams.body));
  console.log('──────────────────────────────────────────────────\n');
  ...
  if (result.hasError) {
    fail('测试失败:', result.errorMsg || '未知错误');
    if (result.content) {
      console.log('\n错误详情:', safeJsonStringify(result.content));
    }
  } else {
    console.log('✅ 测试成功!\n');
    ...
    if (response && response.headers) {
      console.log('\n📋 响应头:');
      console.log(safeJsonStringify(response.headers).substring(0, 500));
    }
    ...
    if (response && response.body) {
      ...
      try {
        ...
        console.log(safeJsonStringify(bodyObj));
      } catch {
        console.log(redactString(response.body).substring(0, 1000));
      }
    }
