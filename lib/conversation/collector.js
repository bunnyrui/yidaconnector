...
function collect(options = {}) {
  ...
  if (input) {
    if (!fs.existsSync(input)) {
      warn(t('exportConv.inputNotFound') || `Input file not found: ${input}`);
      process.exit(1);
    }
    ...
    if (input.endsWith('.jsonl')) {
      ...
      warn(t('exportConv.unsupportedFormat') || 'Unsupported file format. Please use .json or .jsonl');
      process.exit(1);
    }
  }
  ...
  if (isClaudeCode) {
    ...
    if (!projectDir) {
      warn(t('exportConv.noClaudeProject') || 'No Claude Code project found for current directory.');
      warn(t('exportConv.useInputHint') || 'Please use --input to specify a conversation file manually.');
      process.exit(1);
    }
    ...
    if (conversations.length === 0) {
      warn(t('exportConv.noConversations') || 'No conversation files found.');
      process.exit(1);
    }
    ...
    if (latest) {
      ...
      warn(t('exportConv.usingLatest') || `Using latest conversation: ${latestConv.name}`);
  ...
  // 场景3：其他环境，提示用户手动指定
  warn(t('exportConv.envNotSupported') || 'Current environment does not support automatic conversation detection.');
  warn(t('exportConv.useInputHint') || 'Please use --input to specify a conversation file manually.');
  process.exit(1);
}
