...
async function exportConversation(options = {}) {
  ...
  try {
    warn(t('exportConv.title') || '📝 导出 AI 对话记录');

    // Step 1: 检测环境
    warn(t('exportConv.detectEnv') || '🔍 检测当前环境...');
    const env = detectEnvironment();
    const toolName = env.activeToolName || 'Unknown';
    warn(t('exportConv.currentTool') || `  当前 AI 工具: ${toolName}`);

    // Step 2: 采集对话记录
    warn(t('exportConv.collecting') || '📥 采集对话记录...');
    ...
    if (!conversationData.messages || conversationData.messages.length === 0) {
      warn(t('exportConv.noConversation') || '❌ 未找到对话记录');
      process.exit(1);
    }

    warn(t('exportConv.messagesFound') || `  找到 ${conversationData.messages.length} 条消息`);

    // Step 3: 获取应用配置
    warn(t('exportConv.loadingConfig') || '⚙️  加载应用配置...');
    const appConfig = tryGetConfig();
    if (appConfig) {
      warn(t('exportConv.appName') || `  应用名: ${appConfig.appName || appConfig.name || '未命名'}`);
    } else {
      warn(t('exportConv.noConfig') || '  未找到应用配置，使用默认值');
    }

    // Step 4: 格式化为 Markdown
    warn(t('exportConv.formatting') || '📄 格式化 Markdown...');
    ...
    // Step 6: 写入文件
    warn(t('exportConv.writing') || '💾 写入文件...');
    fs.writeFileSync(outputPath, markdown, 'utf-8');

    // 输出成功信息
    warn('');
    warn(t('exportConv.success') || '✅ 导出成功！');
    warn(t('exportConv.outputPath') || '📁 输出路径: ' + outputPath);
    ...
    warn('');
    warn(t('exportConv.error') || '❌ 导出失败: ' + error.message);
    process.exit(1);
  }
}
