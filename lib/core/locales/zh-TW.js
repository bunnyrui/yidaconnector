...
指令：
  ...
  logout                                                       登出 / 切換帳號
  create-app "<名稱>" [描述] [圖示] [顏色] [主題色] [導覽風格] [版面]  建立應用，輸出 appType
  create-page <appType> "<頁面名>"                             建立自訂頁面，輸出 pageId
  create-form create <appType> "<表單名>" <欄位JSON> [--layout <版面>] [--theme <主題>] [--label-align <對齊>]  建立表單頁面
  ...
  connector list-connections <connector-id>                    列出鑑權帳號
  connector create-connection <connector-id> <name> [選項]     建立鑑權帳號
  connector smart-create --curl "curl指令" [選項]              從 cURL 智慧建立連接器
  connector parse-api [選項]                                   解析介面資訊
  ...
  dws <command> [args]                                         釘釘 CLI（通訊錄/行事曆/待辦/審批等）
...
範例：
  ...
  openyida doctor --production --app APP_XXX      正式應用診斷
