...
async function corpGet(path, params, authRef = getAuthRef()) {
  return createYidaClient({ authRef }).get(path, auth => buildCommonParams(auth, params));
}

async function corpPost(path, params, authRef = getAuthRef()) {
  return createYidaClient({ authRef }).postForm(path, auth => buildCommonParams(auth, params));
}
