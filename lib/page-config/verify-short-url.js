...
function parseArgs() {
  ...
  if (args.length < 3) {
    warn(t('verify_short_url.usage'));
    warn(t('verify_short_url.example'));
    warn(t('verify_short_url.formats_label'));
    warn(t('verify_short_url.format_open'));
    warn(t('verify_short_url.format_share'));
    process.exit(1);
  }
  const url = args[2];
...
async function main() {
  ...
  try {
    ...
    warn(t('verify_short_url.validate_failed', err.message));
    process.exit(1);
  }
  ...
  if (result && !result.__needLogin && !result.__csrfExpired) {
    if (result.success && result.content) {
      warn(t('verify_short_url.url_available'));
      ...
      warn(t('verify_short_url.url_taken'));
      console.log(JSON.stringify({
        available: false,
        url: url,
        urlType: urlType,
    ...
    warn(t('verify_short_url.verify_failed'));
    process.exit(1);
  }
}
