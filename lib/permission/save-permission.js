...
function parseArgs(args) {
  if (args.length < 2) {
    warn('用法: openyida save-permission <appType> <formUuid> [--create --name <名称>] [--data-permission <json>] [--action-permission <json>] [--field-permission <json>] [--members <userIds>]');
    warn('示例（更新）: openyida save-permission APP_XXX FORM-XXX --data-permission \'{"role":"DEFAULT","dataRange":"SELF"}\'');
    warn('示例（新增）: openyida save-permission APP_XXX FORM-XXX --create --name "只读权限组" --members "54255850977641"');
    process.exit(1);
  }
  ...
  for (let index = 2; index < args.length; index++) {
    if (args[index] === '--create') {
      ...
      try {
        ...
        warn(`❌ --data-permission 参数 JSON 解析失败: ${args[index + 1]}`);
        process.exit(1);
      }
      index++;
      ...
      try {
        ...
        warn(`❌ --action-permission 参数 JSON 解析失败: ${args[index + 1]}`);
        process.exit(1);
      }
      index++;
      ...
      try {
        ...
        warn(`❌ --field-permission 参数 JSON 解析失败: ${args[index + 1]}`);
        process.exit(1);
      }
      index++;
  ...
  if (createMode && !groupName) {
    warn('❌ 新增模式（--create）必须同时提供 --name <权限组名称>');
    process.exit(1);
  }

  if (!createMode && !dataPermission && !actionPermission && !fieldPermission && !members) {
    warn('❌ 请至少提供 --data-permission、--action-permission、--field-permission 或 --members 参数之一');
    process.exit(1);
  }
