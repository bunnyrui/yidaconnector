...
function ensureAuth() {
  ...
  if (!authRef.cookies || authRef.cookies.length === 0 || !authRef.csrfToken) {
    ...
    throw new CliError(t('preview_process.no_login'), {
      code: 'NEED_LOGIN',
    });
  }
  return authRef;
...
async function fetchProcessInstance(auth, appType, processInstanceId) {
  ...
  const result = await createYidaClient({ authRef: auth }).get(requestPath, { processInstanceId });

  if (!result || !result.success) {
    const errorMsg = (result && result.errorMsg) || t('preview_process.fetch_instance_failed');
    warn(`❌ ${errorMsg}`);
    throw new CliError(errorMsg, {
      code: 'PREVIEW_PROCESS_FETCH_INSTANCE_FAILED',
...
function parseFromActionExecutors(executors, nodes, edges, currentTaskNodes, instanceStatus, _recordsByActivity) {
...
function parseFromOperationRecords(records, nodes, edges, currentTaskNodes, _instanceStatus) {
  let prevNodeId = 'start';
...
async function run(args) {
  ...
  if (!appType || !processInstanceId) {
    ...
    throw new CliError(t('preview_process.usage'), {
