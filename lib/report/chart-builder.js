...
function validateChartConfig(chart, chartIndex) {
  ...
  if (['bar', 'line', 'pie', 'funnel', 'radar', 'heatmap', 'wordcloud', 'map'].includes(chartType)) {
    if (!chart.xField && !chart.yField) {
      warn(`⚠️  ${chartLabel}: 缺少 xField 和 yField，图表将没有数据字段。`);
      hasError = true;
    }
  }
  ...
  if (chartType === 'table') {
    if (!chart.columnFields && !chart.columns && !chart.fields) {
      warn(`⚠️  ${chartLabel}: 缺少 columnFields，表格将没有列。`);
      hasError = true;
    }
  }
  ...
  if (chartType === 'indicator') {
    if (!chart.kpi && !chart.kpiField && !chart.yField && !chart.fields) {
      warn(`⚠️  ${chartLabel}: 缺少 kpi/kpiField 字段，指标卡将没有指标数据。`);
      hasError = true;
    }
  }
  ...
  if (chartType === 'combo') {
    if (!chart.xField) {
      warn(`⚠️  ${chartLabel}: 缺少 xField，组合图需要横轴字段。`);
    }
    if (!chart.leftYFields && !chart.rightYFields) {
      warn(`⚠️  ${chartLabel}: 缺少 leftYFields/rightYFields，组合图需要至少一个纵轴字段。`);
      hasError = true;
    }
  }
  ...
  if (chartType === 'pivot') {
    if (!chart.columnList && !chart.columns) {
      warn(`⚠️  ${chartLabel}: 缺少 columnList，透视表将没有列。`);
      hasError = true;
    }
  }
  ...
  if (chartType === 'gauge') {
    if (!chart.valueField && !chart.yField) {
      warn(`⚠️  ${chartLabel}: 缺少 valueField/yField，仪表盘将没有数值。`);
      hasError = true;
    }
  }
